...
//     4   5
        ...
        weaponPaths = findWeaponPaths(weaponTreeData, 0, "")
    }

    fun findWeaponPaths(node: TreeNode<WeaponBase>, depth: Int, formatting: String) : MutableList<MutableList<TreeNode<WeaponBase>>> {
        val paths: MutableList<MutableList<TreeNode<WeaponBase>>> = mutableListOf()
        node.value.depth = depth

        if (depth == 1) {
            node.value.formatting = formatting.plus("|-")
        } else if (depth > 1){
            node.value.formatting = formatting.plus("-")
        }
        ...
        if (node.getChlidren().isEmpty()) {
            ...
            node.getChlidren().forEach {
                val pathLists = findWeaponPaths(it, node.value.depth + 1, node.value.formatting)
                pathLists.forEach {
                    it.add(0, node)
                    paths.add(it)
                }
